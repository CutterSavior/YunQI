    ...
    public function queueStatus()
    {
        ...
        if($this->request->isPost()){
            ...
            if($status){
                ...
                // 更穩定地定位 PHP 可執行檔
                $os = PHP_OS; // 例如: WINNT, Linux, Darwin
                $phpBinary = (defined('PHP_BINARY') && PHP_BINARY) ? PHP_BINARY : '';
                if(!is_file($phpBinary)){
                    $phpBinary = rtrim(PHP_BINDIR, DIRECTORY_SEPARATOR)
                        . DIRECTORY_SEPARATOR
                        . (stripos($os, 'WIN') === 0 ? 'php.exe' : 'php');
                }
                ...
                // 加上引號以避免路徑空白問題
                if (stripos($os, 'WIN') === 0){
                    $cmd='"'.$phpBinary.'" "'.$command.'"';
                } else {
                    $cmd=$phpBinary.' '.escapeshellarg($command);
                }
